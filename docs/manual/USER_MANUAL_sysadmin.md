...
2. [사용자 관리](#2-사용자-관리)
3. [시스템 설정](#3-시스템-설정)
4. [개선요청 게시판](#4-개선요청-게시판)
5. [경영진 대시보드](#5-경영진-대시보드)
6. [시스템 운영 가이드](#6-시스템-운영-가이드)
7. [Google Sheets 구조](#7-google-sheets-구조)
8. [트러블슈팅](#8-트러블슈팅)
...
시스템 운영 중 발생한 버그, 개선요청, 기능추가 요청을 관리합니다.
...
### 4.2 상태 관리 (sysadmin 전용)

상세 화면에서 **[상태 변경]** 버튼을 통해 상태를 관리합니다.
...
| 상태 | 색상 | 설명 | 운영 가이드 |
|------|------|------|------------|
| 접수 | 🔵 파랑 | 새로 등록된 요청 | 신규 요청, 검토 필요 |
| 검토중 | 🟡 노랑 | 담당자가 검토 중 | 요청 타당성 검토 단계 |
| 진행중 | 🟠 주황 | 개발/수정 진행 중 | 개발 착수됨 |
| 완료 | 🟢 초록 | 처리 완료 | 배포 완료 |
| 보류 | ⚪ 회색 | 일시 보류 | 우선순위 낮음/대기 |
| 반려 | 🔴 빨강 | 처리 불가 | 사유 기재 필수 |
...
1. **접수 후 24시간 내 검토**: 신규 요청은 빠르게 검토하여 사용자에게 피드백
2. **진행중 상태 관리**: 개발 착수 시 상태 변경 및 처리예정일 설정
3. **반려 시 사유 명확히**: 사용자가 이해할 수 있도록 구체적 사유 기재
4. **정기 리뷰**: 보류 상태 요청들 정기적으로 재검토
