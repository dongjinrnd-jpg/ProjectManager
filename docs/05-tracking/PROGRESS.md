...
- [x] **다운로드** ✅ (2026-04-27)
  ...
  - [x] 엑셀 다운로드 (.xlsx) ✅ (2026-04-27)
    ...
    - [x] 미리보기 모달 + 페이지 인쇄 버튼 좌측에 [📊 엑셀 다운로드] 버튼 (Excel 녹색 #217346) ✅
    - [x] 웹 양식과 동일: 타이틀/서브타이틀, 같은 구분 셀 세로 병합, 고객사·ITEM 2줄 한 셀, 본문 셀 줄바꿈, 검정 테두리 ✅
    - [x] 컬럼 너비: No(5)/구분(10)/고객사·ITEM(22)/내용(58) ✅
    - [x] 공지사항은 같은 시트 하단에 통합 (한 행 띄움 + 회색 헤더 + 라인별 행) ✅
    - [x] 인쇄 설정: A4 / 좁은 여백 / 가로 1페이지 맞춤 / 헤더 행 반복 ✅
    - [x] xlsx-js-style 라이브러리 도입 (스타일 지원) ✅
...
- [x] **일정 데이터 다운로드** ✅
  ...
  - [x] **간트 시각 다운로드 (월·주차 2단) (type=gantt-month)** ✅ (2026-05-07, 주차 추가 2026-05-08)
    - [x] 프로젝트 상세에서 간트 뷰 선택 후 [📥 Excel] 클릭 시 월·주차 셀 간트로 다운로드 ✅
    - [x] **2단 헤더**: 월(YY.MM, 5주 가로 병합) + 주차(1주~5주) ✅
    - [x] 한 달 5주 고정 분할 (1주=1~7 / 2주=8~14 / 3주=15~21 / 4주=22~28 / 5주=29~말일) ✅
    - [x] 좌측 고정 컬럼: No, 단계, 항목명, 계획시작, 계획종료, 구분, 상태 ✅
    - [x] **세부추진항목당 2행(계획/실적)**, 동일 단계 연속 묶음 세로 병합 ✅
    - [x] 색상: 계획(연파랑) / 실적-진행중(주황) / 실적-완료(초록) / 실적-지연(빨강) ✅
    - [x] 전체 일정 페이지(/schedules) — 프로젝트 대일정 × 주차 컬럼(단일 색상) ✅
    - [x] 첫 행/좌측 컬럼 freeze, xlsx-js-style 라이브러리로 셀 채움 적용 ✅
    - [x] 파일명: 간트월_프로젝트명_YYYYMMDD.xlsx, 전체간트월_YYYYMMDD.xlsx ✅
