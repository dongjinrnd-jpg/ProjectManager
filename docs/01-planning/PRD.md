...
- [ ] 마일스톤 설정 (주요 일정 표시)
- [ ] 일정 변경 이력 관리
- [ ] **간트차트 드릴다운 (기간별 진행현황 조회):**
  - 전체 간트차트에서 특정 프로젝트의 월/기간 클릭 시 팝업 표시
  - 팝업 내용:
    - 선택 기간 정보 (프로젝트명, 기간)
    - 해당 기간 주간 보고 목록 (주차, 주요 추진 실적 및 계획 요약)
    - 해당 기간 단계 변경 이력
    - 세부추진항목 진행 현황
  - 팝업에서 주간 보고 클릭 시 해당 주차 상세 보기 가능
  - 권한: 모든 사용자 조회 가능
...
- [ ] **경영진 대시보드 (Executive Dashboard):**
  ...
  - **프로젝트 상세 팝업 (경영진용):**
    ...
    - 최근 진행사항 (주간보고 또는 업무일지 기반, 최근 3~5건)
    - 이슈사항 목록
    - 경영진 코멘트 리스트 + 팀장 답변 (스레드 형식)
    - 코멘트 작성 기능
  - **계획 vs 실적 비교 페이지:**
    - 즐겨찾기 프로젝트의 간트차트 (계획/실적 비교)
    - 일정 지연 프로젝트 하이라이트 (빨간색 표시)
    - 단계별 완료율 시각화
    - 프로젝트별 코멘트 작성 기능
  - **접근 권한:** 경영진, 관리자
...
- [ ] 특정 프로젝트 접근 제한 (선택)
- [ ] **경영진 코멘트 기능:**
  - 경영진 메인 대시보드: 프로젝트 카드에서 코멘트 작성/조회
  - 계획 vs 실적 비교: 프로젝트별 코멘트 작성/조회
  - 코멘트 알림: 해당 팀장에게 알림 발송
